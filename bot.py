...
# ==================== মাল্টিপল চ্যানেল চেক ====================
async def is_user_member_all_channels(user_id: int) -> tuple:
    """সব চ্যানেলে জয়েন করেছে কিনা চেক করে"""
    not_joined = []
    for channel in FORCE_CHANNELS:
        try:
            ...
            logging.error(f"চ্যানেল চেক এরর {channel}: {e}")
            not_joined.append(channel)
    
    return len(not_joined) == 0, not_joined

# ==================== পার্মানেন্ট বাটন (নিচে থাকবে) ====================
def get_permanent_keyboard():
    ...
    # চ্যানেল জয়েন বাটন
    for channel in FORCE_CHANNELS:
        keyboard.append([InlineKeyboardButton(f"📢 জয়েন চ্যানেল", url=channel["url"])])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_member:
        # যেসব চ্যানেলে জয়েন নাই সেগুলোর বাটন
        join_buttons = []
        for channel in not_joined:
            join_buttons.append([InlineKeyboardButton(f"📢 জয়েন চ্যানেল", url=channel["url"])])
        ...
        if referrer:
            ...
            try:
                await bot.send_message(
                    chat_id=referrer,
                    text=f"🎉 **নতুন রেফার!**\n\n"
                         f"আপনার রেফার লিংক থেকে {user.first_name} জয়েন করেছেন!\n"
                         f"🆔 ইউজার আইডি: `{user_id}`\n"
                         f"💰 আপনি {REFERRAL_REWARD}টি ক্রেডিট পেয়েছেন!\n"
                         f"👥 মোট রেফার: {user_data['total_referrals']} জন",
                    parse_mode="Markdown",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_join":
        ...
        await query.edit_message_text(
            ...
            f"টাইটেল: {PRANK_IDS.get(prank_id)}\n\n"
            f"নাম্বার লিখুন:\n"
        ...
        text = f"📢 **রেফার লিংক**\n\n"
        text += f"প্রতি রেফারে {REFERRAL_REWARD} ক্রেডিট!\n\n"
        text += f"🔗 `https://t.me/{BOT_USERNAME.replace('@', '')}?start={user_id}`\n\n"
        text += f"👥 রেফার: {user_data['total_referrals']} জন\n"
...
def main():
    ...
    print(f"📢 চ্যানেল: {len(FORCE_CHANNELS)}টি")
